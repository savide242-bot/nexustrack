...
Deno.serve(async (req) => {
  ...
  try {
    const { short_token, action } = await req.json();
    ...
    if (action === "exchange" && short_token) {
      const url = `https://graph.facebook.com/v21.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${appId}&client_secret=${appSecret}&fb_exchange_token=${short_token}`;
      ...
      // Now fetch ad accounts with the long-lived token
